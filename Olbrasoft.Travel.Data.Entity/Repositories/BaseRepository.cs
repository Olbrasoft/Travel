...
namespace Olbrasoft.Travel.Data.Entity.Repositories
{
    ...
    public class BaseRepository<T> : SharpRepository.EfRepository.EfRepository<T>, IBaseRepository<T> where T : class
    {
        ...
        {
            return Queryable.Min(Context.Set<T>(), selector);
        }

        public new IEnumerable<TResult> GetAll<TResult>(Expression<Func<T, TResult>> selector)
        {
            return Queryable.Select(AsQueryable(), selector).ToArray();
        }
        ...
        public bool Exists(bool clearRepositoryCache = false)
        {
            ...
            {
                _exist = Queryable.Any<T>(AsQueryable());
            }
            return _exist;
        }
        ...
        {
            var query = Queryable.Where(Context.Set<T>(), predicate);
